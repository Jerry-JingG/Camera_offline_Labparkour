...
CSV_FIELDS = (
    ...
    + [f"root_pos_w_{axis}" for axis in ("x", "y", "z")]
    + [f"root_lin_vel_w_{axis}" for axis in ("x", "y", "z")]
    + [f"root_lin_vel_b_{axis}" for axis in ("x", "y", "z")]
...
class IsaacPlayTraceRecorder:
    def __init__(
        ...
        print(f"[INFO] Recording IsaacSim obs/action CSV: {self.csv_path}")
        print(f"[INFO] Recording IsaacSim depth trace: {self.depth_dir}")

    def _default_depth_dir(self) -> Path:
        stem = self.csv_path.stem
        if stem.startswith("isaacsim_obs_action_"):
            stem = "isaacsim_depth_" + stem[len("isaacsim_obs_action_") :]
        else:
            stem = stem + "_depth"
    ...
    def write(
        ...
        *,
        ...
        depth_trace: dict | None = None,
        root_state_w=None,
        root_lin_vel_b=None,
    ) -> bool:
        if self.max_steps and policy_step_index >= self.max_steps:
            return False

        obs_np = _select_batch(_to_numpy(proprio_obs), self.record_env_id).reshape(-1)
        action_np = _select_batch(_to_numpy(policy_action), self.record_env_id).reshape(-1)
        depth_yaw_np = _select_batch(_to_numpy(depth_yaw), self.record_env_id).reshape(-1)
        policy_obs_6_8_np = _select_batch(_to_numpy(policy_obs_6_8), self.record_env_id).reshape(-1)
        root_state_np = _select_batch(_to_numpy(root_state_w), self.record_env_id).reshape(-1)
        root_lin_vel_b_np = _select_batch(_to_numpy(root_lin_vel_b), self.record_env_id).reshape(-1)
        ...
        for i, axis in enumerate(("x", "y", "z")):
            row[f"root_pos_w_{axis}"] = _value_or_nan(root_state_np, i)
            row[f"root_lin_vel_w_{axis}"] = _value_or_nan(root_state_np, 7 + i)
            row[f"root_lin_vel_b_{axis}"] = _value_or_nan(root_lin_vel_b_np, i)
        ...
        foot_isaac = _normalize_foot_forces(foot_force_isaac, self.record_env_id)
        ...
        if depth_trace is not None and policy_step_index % self.depth_every == 0:
            row.update(self._write_depth(policy_step_index, depth_trace))
    ...
    def _write_depth(self, policy_step_index: int, depth_trace: dict) -> dict[str, str]:
        ...
        raw_depth = _squeeze_image(_select_image_batch(_to_numpy(depth_trace.get("raw_depth_m")), self.record_env_id))
        processed_capture = _squeeze_image(
            _select_image_batch(_to_numpy(depth_trace.get("processed_capture_depth")), self.record_env_id)
        )
        policy_input = _squeeze_image(
            _select_image_batch(_to_numpy(depth_trace.get("policy_depth_input")), self.record_env_id)
        )

        raw_depth = raw_depth.astype(np.float32)
        processed_capture = processed_capture.astype(np.float32)
        policy_input = policy_input.astype(np.float32)

        np.savez_compressed(capture_path, raw_depth_m=raw_depth, processed_capture_depth=processed_capture)
        np.savez_compressed(policy_input_path, policy_depth_input=policy_input)
...
def _select_batch(values: np.ndarray, env_id: int) -> np.ndarray:
    arr = np.asarray(values)
    if arr.ndim >= 2 and arr.shape[0] > env_id:
        return arr[env_id]
    return arr


def _select_image_batch(values: np.ndarray, env_id: int) -> np.ndarray:
    arr = np.asarray(values)
    if arr.ndim >= 4 and arr.shape[0] > env_id:
        return arr[env_id]
    if arr.ndim == 3 and arr.shape[-1] not in (1, 3, 4) and arr.shape[0] > env_id:
        return arr[env_id]
    return arr
...
def _normalize_foot_forces(value, env_id: int) -> list[float]:
    if value is None:
        return [float("nan")] * 4
    values = _select_batch(_to_numpy(value), env_id).reshape(-1)
    if values.size < 4:
        return [float("nan")] * 4
