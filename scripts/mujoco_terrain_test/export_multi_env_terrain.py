#!/usr/bin/env python3
"""
Export multi-env parkour terrain for MuJoCo based on IsaacLab Jingg branch config.

Terrain types (matching new config with beam):
- parkour_gap: 20%
- parkour_hurdle: 20%  
- parkour_flat: 20%
- parkour_step: 20%
...
NUM_GOALS = 10  # Number of goals/obstacles per terrain

# Terrain type proportions (matching new config with beam, no parkour slope)
TERRAIN_PROPORTIONS = {
    "parkour_gap": 0.2,
    "parkour_hurdle": 0.2,
    "parkour_flat": 0.2,
...
class TerrainConfig:
    """Configuration matching IsaacLab's extreme_parkour_terrains_cfg (Jingg branch)."""
    def __init__(self):
...
def generate_gap_terrain(cfg: TerrainConfig, difficulty: float) -> tuple:
    ...
    return height_field, []  # No extra meshes


def generate_hurdle_terrain(cfg: TerrainConfig, difficulty: float, apply_flat: bool = False) -> tuple:
...
def generate_step_terrain(cfg: TerrainConfig, difficulty: float) -> tuple:
...
def generate_beam_terrain(cfg: TerrainConfig, difficulty: float) -> tuple:
    ...
    Beams are represented as elevated ground sections that the robot must pass under.
    For MuJoCo hfield, we represent beams by creating tall obstacles on the sides
    with a gap in the middle for the robot to pass through.
    
    Returns:
        height_field: Ground heightfield
        beam_boxes: List of (x, y, z, size_x, size_y, size_z) for suspended beams
    """
    width_pixels = int(cfg.size[0] / cfg.horizontal_scale)
    length_pixels = int(cfg.size[1] / cfg.horizontal_scale)
    height_field = np.zeros((width_pixels, length_pixels))
    ...
    beam_height_min = 0.35 - 0.15 * difficulty
    beam_height_max = 0.45 - 0.15 * difficulty
    ...
    for i in range(num_beams):
        ...
        beam_z_m = platform_height * cfg.vertical_scale + clearance + cfg.beam_depth / 2
        ...
        beam_boxes.append((beam_x_m, beam_y_m, beam_z_m, beam_size_x, beam_size_y, beam_size_z))
...
def generate_multi_env_terrain(
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            block, extra = generator(cfg, difficulty)
            ...
            if terrain_type == "parkour_beam" and extra:
                for (bx, by, bz, sx, sy, sz) in extra:
...
'''
    ...
    # Add beam obstacles
    for i, (bx, by, bz, sx, sy, sz) in enumerate(beams):
        # Convert to MuJoCo coordinates (centered at origin)
        mx = bx - half_x
        my = by - half_y
        mz = bz - min_height
        xml_content += f'''
...
def main():
    ...
    print("=" * 70)
    print("Multi-Env Parkour Terrain Export (IsaacLab Jingg branch - with BEAM)")
    print("=" * 70)
